...
| serving.cacheClaim | string | `"hf-cache"` | Name of the model cache `PersistentVolumeClaim` in the serving namespace (the connectivity chart's `modelServing.cache.pvc.name`). The claim outlives the pool and is one volume bound in one zone: `create_node_pool` pins a pool's nodes to that zone (the gpu-node-pool chart's `pool.zones`) and `list_clusters` names the claim and its zone under serving readiness. |
