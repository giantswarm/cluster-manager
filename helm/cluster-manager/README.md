...
meta chart (`components.cluster-manager.enabled`), which sets `backend`, the
...
- Downloads run as Jobs in the serving namespace with the KServe
  ...
  the node annotation `cluster-manager.giantswarm.io/memory-budget-gib: "96"`
...
- The cache inventory (`kserve.inventory.*`) reads each cache node either with
  ...
  the serving namespace that runs `cluster-manager cache-agent` — the same
  ...
  on its node only); `networkPolicy.enabled` then also allows cluster-manager →
...
- The inventory names a cache directory after the repository known to have
  ...
  ConfigMap `kserve.cache.indexConfigMap` (default `cluster-manager-cache-index`)
  in the serving namespace, in which cluster-manager records, while an
...
**Homepage:** <https://github.com/giantswarm/cluster-manager>
...
* <https://github.com/giantswarm/cluster-manager>
