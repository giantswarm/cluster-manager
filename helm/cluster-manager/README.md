...
| github.enabled | bool | `false` | Register with muster pinned to the GitHub App `giantswarm-cluster-manager` and offer commit mode (`mode: commit` on `create_node_pool` and `delete_node_pool`: the pull request opened as the person in the repository that owns the cluster). The MCPServer's auth then pins the App as the authorization server — muster runs the App's consent once per person and puts their user token on every call, which this server verifies with `GET /user` and opens the pull request with — and sets `forwardIdentity: true`: muster forwards the person's IdP ID token in `X-Muster-Id-Token` next to it, validated as `oauth` validates a forwarded token, so apply mode keeps acting on Kubernetes as the person. One registration carries both tokens. Needs `oauth.enabled`. Off: muster forwards the ID token as the bearer (`muster.mcpServer.auth.forwardToken`) and commit mode is refused. |
