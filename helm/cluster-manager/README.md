...
| serving.namespace | string | `"model-serving"` | Namespace on a serving cluster where the LLMInferenceServices go, named as `target.servingNamespace` in the registered `kserve` backend. |
| serving.cacheClaim | string | `"hf-cache"` | Base name of the model cache `PersistentVolumeClaim`s in the serving namespace (the connectivity chart's default `modelServing.cache.pvc.name`). A claim outlives the pool and is one volume bound in one zone, so the cache is one claim per zone: `create_node_pool` with `zones` composes the zone's claim into the pool's slice — the claim Bound there, else `<cacheClaim>-<zone>` — and pins the pool's nodes to the zone (the gpu-node-pool chart's `pool.zones`); `list_clusters` names every claim with its zone under serving readiness. |
| serving.sliceChartVersion | string | `""` | Exact `agent-platform` chart version the `<cluster>-agent-platform` slice release pins. Empty pins the version the installation's own platform release runs (at least 4.44.0, the first chart whose serving slice is the llm-d control plane alone); set it only for a lab running an unreleased chart. |
| serving.certificateIssuer | string | `"letsencrypt-giantswarm"` | cert-manager `ClusterIssuer` the `<cluster>-agent-platform` slice release asks for the certificate of `models.<domain>` when the platform's wildcard is not usable: on a workload cluster (another domain) and on the installation's own cluster when the platform's release names no `gatewayApi.gateway.tls.secretName` (TLS terminated at the fleet's edge Gateway, its wildcard in another namespace). `letsencrypt-giantswarm` is the issuer every Giant Swarm cluster runs. Empty composes none — the connectivity chart then refuses a models Gateway without a certificate. |
| gpuOperator.dcgmExporter | bool | `false` | Run NVIDIA's DCGM exporter on the GPU pools' nodes through the composed `<cluster>-gpu-operator` release (its `dcgmExporter.enabled`), for an installation whose observability scrapes it (a ServiceMonitor on the `nvidia-dcgm-exporter` Service in `kube-system`). Off by default: nothing scrapes the exporter out of the box, and on a fresh pool node it is an image pull and a pod initialising DCGM on the GPU while the device plugin brings `nvidia.com/gpu` up. |
| flux.tenantServiceAccount | string | `"automation"` | ServiceAccount in every org namespace the installation's Flux runs a composed release under when the release's objects live in that namespace: the pool release (its Cluster API objects) and the `<cluster>-agent-platform` slice release (its child HelmReleases). Giant Swarm installations enforce the `flux-multi-tenancy` policy on every HelmRelease in `org-<org>` — a `serviceAccountName` or a kubeconfig on each —, and `automation` is the org's tenant ServiceAccount there. A release installing into a cluster (the `<cluster>-gpu-operator` release, the slice's children) is delivered through the cluster's `<cluster>-kubeconfig` Secret instead, the installation's own cluster included. Empty renders none, for a lab without the policy. |
| mcp.enabled | bool | `true` | Serve the MCP streamable-HTTP endpoint (the server's only surface). |
| mcp.path | string | `"/mcp"` | MCP endpoint path: every tool, the main MCPServer's. |
| mcp.commitPath | string | `"/commit/mcp"` | MCP endpoint path of the commit registration, served with `github.enabled`: the tools with a commit mode behind the GitHub App's user token, the `<name>-commit` MCPServer's. |
...
| github.enabled | bool | `false` | Offer commit mode (`mode: commit` on `create_node_pool` and `delete_node_pool`: the pull request opened as the person in the repository that owns the cluster) through a second muster registration, the MCPServer `<muster.mcpServer.name>-commit` on `mcp.commitPath`, which serves those two tools and `get_info`. Its auth pins the GitHub App `giantswarm-cluster-manager` as the authorization server — muster runs the App's consent once per person and puts their user token on every call, which this server verifies with `GET /user` and opens the pull request with — and sets `forwardIdentity: true`: muster forwards the person's IdP ID token in `X-Muster-Id-Token` next to it, validated as `oauth` validates a forwarded token, so Kubernetes is still acted on as the person. The main MCPServer stays the one that forwards the ID token as the bearer (`muster.mcpServer.auth.forwardToken`) and serves every tool without the App's consent; `mode: commit` there is refused, naming the commit registration and its login. Needs `oauth.enabled`. Off: commit mode is refused. |
| github.apiURL | string | `"https://api.github.com"` | GitHub's REST API, for `GET /user` and the pull request. |
| github.authorizationServer.issuer | string | `"https://github.com/apps/giantswarm-cluster-manager"` | The issuer identity the person's grant is filed under: the App's own, so the login App's GitHub grant stays separate. Also passed to the server, which names it in its refusals. |
| github.authorizationServer.expectedIssuer | string | `"https://github.com/login/oauth"` | The issuer GitHub puts in the RFC 9207 `iss` parameter of its authorization response, `https://github.com/login/oauth` for every App. Empty omits the field. |
| github.authorizationServer.authorizationEndpoint | string | `"https://github.com/login/oauth/authorize"` | GitHub's authorize endpoint (GitHub publishes no discovery document). |
| github.authorizationServer.tokenEndpoint | string | `"https://github.com/login/oauth/access_token"` | GitHub's token endpoint. |
| github.authorizationServer.scopes | string | `""` | The OAuth `scope` parameter, only for an authorization server that needs one (a lab's stand-in); empty for GitHub, whose App permissions are the App's. Empty omits the field. |
| github.authorizationServer.clientCredentialsSecretRef.name | string | `"giantswarm-cluster-manager-oauth-client"` | Secret with the App's OAuth client under `client-id` and `client-secret`. |
