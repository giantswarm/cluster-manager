...
environment variable named next to it; flags win over the environment.`,
	...
	f.StringVar(&o.servingCacheClaim, "serving-cache-claim", envOr("CLUSTER_MANAGER_SERVING_CACHE_CLAIM", tools.DefaultCacheClaimName), "Name of the model cache PersistentVolumeClaim in the serving namespace (the connectivity chart's modelServing.cache.pvc.name): create_node_pool pins a pool's nodes to the zone its volume is bound to, list_clusters names it (CLUSTER_MANAGER_SERVING_CACHE_CLAIM)")
