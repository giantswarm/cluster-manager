...
func NewMCPServer(svc *tools.Service, version string) *mcpserver.MCPServer {
	...
	s.AddTool(mcp.NewTool(ToolCreateNodePool,
		...
		mcp.WithArray(argZones, mcp.Items(map[string]any{"type": "string"}), mcp.Description("The availability zone the pool's nodes are pinned to (pool.zones; [\"eu-central-1a\"]), among the zones of the cluster's node subnets — a zone the cluster has no node subnet in is refused, naming the zones it has. With the cache on, one zone: the pool's slice mounts that zone's model cache claim (the claim Bound there, else hf-cache-<zone>, created by the connectivity chart and kept), so the zone is chosen by capacity and brings its own cache; several zones are refused, a claim being one volume in one zone (with cache false several zones are allowed). Default none: the serving namespace's claims decide — one claim Bound pins the pool to its zone and the slice mounts it; several claims are refused, asking for zones. A re-run moves the pin and the claim.")),
