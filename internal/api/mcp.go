...
func NewMCPServer(svc *tools.Service, version string) *mcpserver.MCPServer {
	...
	s.AddTool(mcp.NewTool(ToolCreateNodePool,
		...
		mcp.WithString(argMode, mcp.Enum(tools.ModeApply, tools.ModeCommit), mcp.DefaultString(tools.ModeApply), mcp.Description("apply: land the objects on the installation as you; commit: open the pull request as you in the git repository that owns the cluster (list_clusters' commitTarget: the releases as files under its cluster-manager/ directory, the kustomization entries added), answered with its URL in commit.pullRequest — the kserve backend registration is written on the installation as in apply; needs the server registered with its GitHub App (get_info modes.commit) and your consent to it in muster, a cluster no Flux Kustomization owns is refused; dryRun shows the files, the repository and the directory")),
	...
	s.AddTool(mcp.NewTool(ToolDeleteNodePool,
		...
		mcp.WithString(argMode, mcp.Enum(tools.ModeApply, tools.ModeCommit), mcp.DefaultString(tools.ModeApply), mcp.Description("apply: remove the objects from the installation as you — refused for a pool whose release Flux still applies from git; commit: open the removal pull request as you (the files of the pool and, with the last pool, of the operator and slice releases, the kustomization entries), the backend registration re-written or removed on the installation; commit.liveSteps names what the merge leaves: on a Kustomization that does not prune, delete_node_pool in mode apply after the merge; a pool that is not in the repository is refused")),
