...
func NewMCPServer(svc *tools.Service, version string) *mcpserver.MCPServer {
	...
	s.AddTool(mcp.NewTool(ToolListNodePools,
		mcp.WithDescription("List the pools of one cluster with their lifecycle: phase (creating, ready, scaling, removing, failed) and steps — the pool release Ready, the MachinePool ready, the nodes (NodeClaims launching, ready, terminating; on a GPU pool of cluster-manager's the ready nodes that hold nothing named idle since their last pod left, which delete_node_pool removes with the pool, and a MachinePool still listing instances the cluster no longer has said so), and for a pool created with prewarm the placeholder (prewarm: its Job's pod pending while the first node launches, holding a node, preempted by the first workload, finished when the hold ended, or absent — the step never decides the phase) — each pending, inProgress, done or failed with since/finishedAt timestamps; a pool release whose MachinePool is not created yet is listed creating, a pool under delete_node_pool stays listed removing with deleting: true and the teardown's pending objects until its HelmRelease is gone; plus the pool's Kubernetes version and the control plane's as two fields (no verdict is drawn), replicas and ready replicas, the instance types when readable, the accelerator of the owning pool release and its sizes (sizes: each size as AWS lists it, what it leaves a predictor, and its on-demand price per hour in the cluster's region — pricePerHourUSD, priceSource, priceAsOf, or priceNote saying why there is none), and the HelmRelease that owns the pool (null for a pool created by other means)."),
