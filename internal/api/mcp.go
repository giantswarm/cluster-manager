...
// CommitToolNames lists the tools the commit MCP server registers: the ones
// with a commit mode, and get_info.
func CommitToolNames() []string {
	return []string{ToolGetInfo, ToolCreateNodePool, ToolDeleteNodePool}
}
...
type Info struct {
	...
	// caller, offered when the server is registered with its GitHub App —
	// through CommitRegistration, the muster registration pinned to it.
	Modes              Modes  `json:"modes"`
	CommitRegistration string `json:"commitRegistration,omitempty"`
	// Tools are the tools of the registration answering: every tool on the
	// main one, the tools with a commit mode on the commit registration.
...
// mainInstructions are the main MCP server's.
const mainInstructions = "Manage the clusters of this Giant Swarm installation and their GPU node pools. list_clusters names every cluster with its organization, release, whether it is the installation's own cluster, whether the GPU operator and the serving layer are present and who provides them, and its GPU pool releases; list_node_pools shows one cluster's MachinePools with the pool's Kubernetes version and the control plane's side by side; create_node_pool composes a GPU pool's release (gpu-node-pool chart) from the cluster's current release and settings and lands it as you — a second call on the same name is the update, dryRun the drift check; delete_node_pool removes what create_node_pool created, the pool's idle nodes first, and refuses while a node of the pool is busy; enable_model_serving and disable_model_serving switch the serving slice (KServe, the models Gateway) on or off for a cluster through its one <cluster>-agent-platform release, with or without a GPU pool; remove_model_cache removes a cluster's model cache — the claims that outlive every pool and are billed while they exist — after switching the slice to serve without it. Every call runs as you: what you may read is what these tools list, what you may write is what they change."

// commitInstructions are the commit MCP server's.
const commitInstructions = "The commit registration of cluster-manager: create_node_pool and delete_node_pool in mode commit open the node pool's pull request in the git repository that owns the cluster, as you, with your GitHub authorization through the server's GitHub App (your consent to it in muster). Every other tool, dry runs and mode apply are on the server's main registration, which needs no GitHub authorization. Every call runs as you."

// NewMCPServer builds the MCP server exposing every tool on the main path,
// the registration that forwards the IdP token. Results are JSON text.
func NewMCPServer(svc *tools.Service, version string) *mcpserver.MCPServer {
	return newMCPServer(svc, version, mainInstructions, ToolNames())
}

// NewCommitMCPServer builds the MCP server of the commit path, the
// registration pinned to the GitHub App: the tools with a commit mode and
// get_info, on the same handlers.
func NewCommitMCPServer(svc *tools.Service, version string) *mcpserver.MCPServer {
	return newMCPServer(svc, version, commitInstructions, CommitToolNames())
}

func newMCPServer(svc *tools.Service, version, instructions string, names []string) *mcpserver.MCPServer {
	...
	t := &handlers{svc: svc, version: version, tools: names}
	all := t.serverTools()
...
// serverTools are every tool's definition and handler by name.
func (h *handlers) serverTools() map[string]mcpserver.ServerTool {
	defs := []mcpserver.ServerTool{
		{Tool: mcp.NewTool(ToolGetInfo,
			mcp.WithDescription("Report this server's version, the write modes it offers (apply: objects landed on the installation as the caller; commit: a pull request opened as the caller, through the muster registration commitRegistration names — the one pinned to the server's GitHub App), the names of the tools this registration serves, and whether the installation serves the Cluster API (cluster.x-k8s.io) the cluster tools read from — absent on an installation without the KaaS components, where list_clusters is empty and the tools naming a cluster refuse."),
			mcp.WithReadOnlyHintAnnotation(true),
		), Handler: h.getInfo},

		{Tool: mcp.NewTool(ToolListClusters,
		...
		{Tool: mcp.NewTool(ToolListNodePools,
		...
		), Handler: h.enableModelServing},

		{Tool: mcp.NewTool(ToolDisableModelServing,
			...
			mcp.WithIdempotentHintAnnotation(true),
			mcp.WithOpenWorldHintAnnotation(true),
		), Handler: h.disableModelServing},

		{Tool: mcp.NewTool(ToolRemoveModelCache,
			...
			mcp.WithIdempotentHintAnnotation(true),
			mcp.WithOpenWorldHintAnnotation(true),
		), Handler: h.removeModelCache},
	}
	out := make(map[string]mcpserver.ServerTool, len(defs))
	for _, d := range defs {
		out[d.Tool.Name] = d
	}
	return out
...
type handlers struct {
	svc     *tools.Service
	version string
	// tools are the names the server registers, as get_info reports them.
	tools []string
}

func (h *handlers) getInfo(ctx context.Context, _ mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return jsonResult(Info{Version: h.version, Modes: Modes{Apply: true, Commit: h.svc.CommitAvailable()}, CommitRegistration: h.svc.CommitRegistration(), Tools: h.tools, ClusterAPI: h.svc.ClusterAPI(ctx)})
}
