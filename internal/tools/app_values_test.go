...
// A ConfigMap's one key is its values whatever its name; the Secrets the
// App names are never read.
func TestAppValuesMergeOrder(t *testing.T) {
	...
	assert.Equal(t, map[string]any{
		...
		"top":       "top",     // priority 150 over spec.userConfig, under the key values.yaml
...
func TestAppValuesRefusals(t *testing.T) {
	for _, tc := range []struct {
		...
		{"several-keys", []string{"values ConfigMap org-acme/several-keys-config carries 2 keys (values, values.yaml)"}},
