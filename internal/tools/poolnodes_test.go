...
func TestHolder(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"InferenceService's predictor without a GPU limit", pod("model-serving", "isvc", map[string]any{"containers": []any{map[string]any{"name": "kserve-container"}}}, running, map[string]any{detect.LabelKServeInferenceService: "mistral-7b"}, nil), "node-1", "model-serving/isvc (KServe predictor)", true},
