...
func TestHolder(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"InferenceService's predictor without a GPU limit", pod("model-serving", "isvc", map[string]any{"containers": []any{map[string]any{"name": "kserve-container"}}}, running, map[string]any{labelKServeInferenceService: "mistral-7b"}, nil), "node-1", "model-serving/isvc (KServe predictor)", true},
		{"LLMInferenceService's workload without a GPU limit", pod("model-serving", "llm", map[string]any{"containers": []any{map[string]any{"name": "main"}}}, running, map[string]any{labelPartOf: partOfLLMISVC}, nil), "node-1", "model-serving/llm (KServe predictor)", true},
