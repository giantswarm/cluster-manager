...
func TestEnableModelServingOwnCluster(t *testing.T) {
	...
	assert.Equal(t, &SliceRelease{Name: "gazelle-agent-platform", Namespace: "org-giantswarm", ChartVersion: "4.27.2", Domain: "gazelle.example.io", ModelsHost: "models.gazelle.example.io", JWKS: "http://dex.giantswarm.svc.cluster.local:5556/keys"}, out.Slice, "the dry run names the JWKS source: the platform's Dex service")
...
func TestEnableModelServingRefusals(t *testing.T) {
	...
	require.NoError(t, unstructured.SetNestedSlice(platform.Object, []any{map[string]any{"chartVersion": "4.25.0"}}, "status", "history"))
	_, err = l.installation.Resource(HelmReleaseGVR).Namespace("flux-giantswarm").Update(ctx, platform, metav1.UpdateOptions{})
	require.NoError(t, err)
	_, err = svc.EnableModelServing(ctx, serving("wc1", false))
	assertRefused(t, err, "flux-giantswarm/agent-platform runs agent-platform chart 4.25.0, below 4.27.0")
	assertNothingLanded(t, l, "wc1-agent-platform")
